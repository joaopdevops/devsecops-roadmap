<div align="right">
  <a href="README.md">🇺🇸 Read in English</a>
</div>

# Roadmap DevOps — João Pedro Ribeiro

**Sargento do Exército Brasileiro → DevOps/Cloud Engineer (em construção) | Brasil**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jo%C3%A3o-pedro-ribeiro-37156a361)
[![GitHub](https://img.shields.io/badge/GitHub-joaopdevops-181717?style=flat-square&logo=github)](https://github.com/joaopdevops)

---

## Sobre

Passei 8 anos no Exército Brasileiro. Ingressei em 2018, participei da Intervenção Federal no Rio de Janeiro e me formei na Escola de Sargentos das Armas (ESA) em 2022. Hoje exerço a função de 3º Sargento de carreira e trabalho na seção de informática da minha OM, administrando servidores Linux e monitoramento com Zabbix no dia a dia.

Em paralelo, curso Redes de Computadores na Estácio (EAD). Em 2025 tirei minha primeira certificação (Oracle OCI Foundations) e decidi tornar a transição oficial com um plano estruturado.

Estou seguindo um plano estruturado de 6 meses para me tornar DevOps/Cloud Engineer, cobrindo Redes (CCNA), Linux, Docker, Kubernetes (CKA), AWS, Terraform e CI/CD. Atualmente estou na fase de Redes, estudando os fundamentos para a certificação CCNA.

---

## Progresso do Roadmap

| Fase | Tema | Status |
|:----:|------|:------:|
| 01 | Redes (CCNA) | 🔄 Em andamento |
| 02 | Linux & Shell Scripting | ⏳ Planejado |
| 03 | Docker | ⏳ Planejado |
| 04 | Kubernetes (CKA) | ⏳ Planejado |
| 05 | AWS | ⏳ Planejado |
| 06 | Terraform | ⏳ Planejado |
| 07 | CI/CD | ⏳ Planejado |

---

## Estrutura do Repositório

```
devops-roadmap/
├── fase-01-redes/
│   ├── resumos/            # Notas em markdown (Obsidian)
│   ├── labs/               # Scripts e exercícios
│   └── projeto-integrador/
├── fase-02-linux/
├── fase-03-docker/
├── fase-04-kubernetes/
├── fase-05-aws/
├── fase-06-terraform/
└── fase-07-cicd/
```

Cada fase segue a mesma organização:
- `resumos/` — anotações de estudo exportadas do Obsidian
- `labs/` — scripts e arquivos de configuração dos laboratórios
- `projeto-integrador/` — projeto prático ao final de cada fase

---

## Stack

### Uso Atualmente

![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Bash](https://img.shields.io/badge/Bash-121011?style=flat-square&logo=gnu-bash&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Zabbix](https://img.shields.io/badge/Zabbix-CC0000?style=flat-square&logoColor=white)

### Aprendendo (plano de 6 meses)

![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white)

---

## Certificações

| Certificação | Status |
|---|:---:|
| Oracle Cloud Infrastructure Foundations Associate | ✅ Mai/2025 |
| CCNA — Cisco Certified Network Associate | 🎯 Em andamento |
| CKA — Certified Kubernetes Administrator | 🎯 Após CCNA |
| AWS Solutions Architect Associate | ⏳ Após CKA |

---

## Metas Atuais

- Seguindo um plano estruturado de 6 meses de DevOps/Cloud
- Candidatura para a **Linux Foundation LiFT Scholarship 2026** — resultado até 30/06/2026
- Próxima certificação: **CCNA**, seguida da **CKA**

---

## Projetos Exploratórios Anteriores

Antes de iniciar o plano estruturado de 6 meses, explorei vários tópicos de DevOps por conta própria. Esses repositórios contêm experimentos iniciais — imperfeitos, mas reais.

| Repositório | Descrição |
|---|---|
| [pre_kubernetes](https://github.com/joaopdevops/pre_kubernetes) | Exploração inicial de conceitos de Kubernetes antes do plano estruturado |
| [learning-terraform](https://github.com/joaopdevops/learning-terraform) | Primeiros experimentos com Terraform e provisionamento de S3 na AWS |
| [CI-CD](https://github.com/joaopdevops/CI-CD) | Experimento inicial de pipeline CI/CD com Terraform, Docker e GitHub Actions |
| [github-actions-simples](https://github.com/joaopdevops/github-actions-simples) | Primeiro pipeline CI com GitHub Actions, Python e pytest |
| [first-image-docker](https://github.com/joaopdevops/first-image-docker) | Minha primeira imagem Docker — Nginx servindo uma página HTML estática |
| [github-access-audit](https://github.com/joaopdevops/github-access-audit) | Script Shell de auditoria de acessos no GitHub, criado durante os estudos de Bash |

---

## Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jo%C3%A3o-pedro-ribeiro-37156a361)
[![GitHub](https://img.shields.io/badge/GitHub-joaopdevops-181717?style=flat-square&logo=github)](https://github.com/joaopdevops)
